# -*- coding: utf-8 -*-
"""Sentiment Analysis_RBC GAM.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1popvBPVYTJ8r8ZwwOjDsENbrhlSIhsT6

# Vader Sentiment Analysis

# Stock Return Analysis
"""

# Google news api library install
!pip install gnews

# Import required library
from gnews import GNews
import pandas as pd
import datetime
import nltk
nltk.download('vader_lexicon')
from nltk.sentiment.vader import SentimentIntensityAnalyzer
sid = SentimentIntensityAnalyzer()
import re
import string
import unicodedata
!pip install unidecode
import unidecode
!pip install yfinance
import yfinance as yf
import scipy.stats as stats

# Define a function called get_news to pull news from googlenews
def get_news(Language='en',Country='US', Company='Microsoft'): 
# Google news setting
    google_news = GNews(language=Language, country=Country)
    news = google_news.get_news(Company)
    news_title=[]
    news_text=[]
    news_date=[]
    news_author=[]
    for i in range(len(news)):
        article = google_news.get_full_article(
        news[i]['url'])
        if article != None: 
            news_title.append(article.title)
            news_text.append(article.text)
            news_author.append(article.authors)
            news_date.append(news[i]['published date'])
    news_df=pd.DataFrame(list(zip(news_title,news_date,news_author,news_text)),columns=['title','publish date', 'author','text'])
    news_df['datetime']=news_df['publish date'].apply(lambda x: datetime.datetime.strptime(x, '%a, %d %b %Y %H:%M:%S %Z'))
    news_df=news_df.drop(columns=['publish date'])
    news_df['date']=news_df['datetime'].apply(lambda x: x.date())
    news_df=news_df.sort_values('date', ascending=False)
    return news_df

# Define a function called get_sentiment_score to score the polarity of news
def get_sentiment_score(news_df):
    def preprocess(text):
        text=text.lower()
        text=re.sub(r'[^\w\s]', '', text)
        text=unidecode.unidecode(text)
        text=re.sub(r'\d+', '', text)
        return text
    news_df['text']=news_df['text'].apply(lambda x: preprocess(x))
    news_df['polarity scores']=news_df['text'].apply(lambda x: sid.polarity_scores(x))
    news_df['compound scores']=news_df['polarity scores'].apply(lambda x: x['compound'])
    news_df_grouped=news_df.groupby('date')
    mean_df=news_df_grouped.mean()
    mean_df=mean_df.reset_index()
    return mean_df

# Define a function called get_stock to get stock price from Yahoo Finance
def get_stock(stocks=['MSFT'],start_date = '2022-05-01',stop_date = '2022-06-30'):
    stock_list = " ".join(stocks)
    table = yf.download(stock_list, start=start_date, end=stop_date)['Adj Close']
    df_stock=pd.DataFrame(table)
    df_stock=df_stock.reset_index().rename(columns={'Adj Close':'Close Price'})
    df_stock['date']=df_stock['Date'].apply(lambda x:x.date())
    df_stock=df_stock.drop(columns=['Date'])
    return df_stock

# Define a function called get_correlation_coefficient to investigate the relationship between the sentiment scores of news and stock return
# stock return = ((P1-P0)+D1)/P0 where D1=0
def get_correlation_coefficient(mean_df,df_stock):
    df_final=pd.merge(mean_df,df_stock,how='left',on='date').dropna()
    df_final['Close Price Last Day']=df_final['Close Price'].shift(1)
    # Define Stock Return 
    df_final['Stock Return']=(df_final['Close Price']-df_final['Close Price Last Day'])/df_final['Close Price Last Day']
    # Define Price Change (unmask below to do the Price Change Analysis)
    #df_final['Stock Return']=df_final['Close Price']-df_final['Close Price Last Day']
    df_final=df_final.dropna()
    r = stats.pearsonr(df_final['compound scores'], df_final['Stock Return'])
    print("Pearson Correlation coefficient:"+str(r[0]))
    return r[0]

# Define a function to get correlation coefficient between news' sentiment score and stock price of companies
def corr_coefficient(company, ticker):
    news_df=get_news(Company=company)
    sentiment_df=get_sentiment_score(news_df)
    start_d=sentiment_df['date'][0]
    end_d=sentiment_df['date'][len(sentiment_df)-1]
    stock_df=get_stock(stocks=ticker,start_date=start_d,stop_date=end_d)
    r=get_correlation_coefficient(sentiment_df,stock_df)
    return r

# Define a function called corr_df to automate the whole sentiment analysis process
def corr_df(companylist,tickers):
  corr_list=[]
  for i in range(len(companylist)):
      corr_list.append(corr_coefficient(companylist[i],[tickers[i]]))
  corr_df=pd.DataFrame(list(zip(companylist,corr_list)),columns=['company','corr_value'])
  return corr_df

# 11 companies sample 
df1=corr_df(['RBC','Microsoft','Amazon','CVR Energy','OXY','Coinbase','IBM','Alpha Metallurgical','Peabody Energy','AAPL','Consol Energy'],['RY','MSFT','AMZN','CVI','OXY','COIN','IBM','AMR','BTU','AAPL','CEIX'])

# Display correlation coefficient between stock return and Vader sentiment scores of selected companies
df1

# ETF sentiment analysis
# 5 interested ETF
df2=corr_df(['iShares US Healthcare','iShares US Consumer Goods','iShares S&P 500 Growth','ARKK','ARKW'],['IYH','IYK','IVW','ARKK','ARKW'])

# Display correlation coefficient between stock return and Vader sentiment scores of selected ETFs

"""# Trading Volume Analysis"""

# Google news api library install
!pip install gnews

# Import required library
from gnews import GNews
import pandas as pd
import datetime
import nltk
nltk.download('vader_lexicon')
from nltk.sentiment.vader import SentimentIntensityAnalyzer
sid = SentimentIntensityAnalyzer()
import re
import string
import unicodedata
!pip install unidecode
import unidecode
!pip install yfinance
import yfinance as yf
import scipy.stats as stats

# Define a function called get_news to pull news from Google News API
def get_news(Language='en',Country='US', Company='Microsoft'): 
# Google News setting
    google_news = GNews(language=Language, country=Country)
    news = google_news.get_news(Company)
    news_title=[]
    news_text=[]
    news_date=[]
    news_author=[]
    for i in range(len(news)):
        article = google_news.get_full_article(
        news[i]['url'])
        if article != None: 
            news_title.append(article.title)
            news_text.append(article.text)
            news_author.append(article.authors)
            news_date.append(news[i]['published date'])
    news_df=pd.DataFrame(list(zip(news_title,news_date,news_author,news_text)),columns=['title','publish date', 'author','text'])
    news_df['datetime']=news_df['publish date'].apply(lambda x: datetime.datetime.strptime(x, '%a, %d %b %Y %H:%M:%S %Z'))
    news_df=news_df.drop(columns=['publish date'])
    news_df['date']=news_df['datetime'].apply(lambda x: x.date())
    news_df=news_df.sort_values('date', ascending=False)
    return news_df

# Define a function called get_sentiment_score to score the polarity of news
def get_sentiment_score(news_df):
    def preprocess(text):
        text=text.lower()
        text=re.sub(r'[^\w\s]', '', text)
        text=unidecode.unidecode(text)
        text=re.sub(r'\d+', '', text)
        return text
    news_df['text']=news_df['text'].apply(lambda x: preprocess(x))
    news_df['polarity scores']=news_df['text'].apply(lambda x: sid.polarity_scores(x))
    news_df['compound scores']=news_df['polarity scores'].apply(lambda x: x['compound'])
    news_df_grouped=news_df.groupby('date')
    mean_df=news_df_grouped.mean()
    mean_df=mean_df.reset_index()
    return mean_df

# Define a function called get_volume to get trading volume from Yahoo Finance API
def get_volume(stocks=['MSFT'],start_date = '2022-05-01',stop_date = '2022-06-30'):
    stock_list = " ".join(stocks)
    table = yf.download(stock_list, start=start_date, end=stop_date)['Volume']
    df_v=pd.DataFrame(table)
    df_v=df_v.reset_index().rename(columns={'Volume':'Trading Volume'})
    df_v['date']=df_v['Date'].apply(lambda x:x.date())
    df_v.drop(columns=['Date'])
    return df_v

# Define a function called get_correlation_coefficient to investigate the relationship between the sentiment scores of news and stock trading volume change
def get_correlation_coefficient(mean_df,df_v):
    df_final=pd.merge(mean_df,df_v,how='left',on='date').dropna()
    df_final['Trading Volume Last Day']=df_final['Trading Volume'].shift(1)
    # define trading volume percentage change 
    df_final['Day Change Percentage']=(df_final['Trading Volume']-df_final['Trading Volume Last Day'])/df_final['Trading Volume Last Day']
    df_final=df_final.dropna()
    df_final
    r = stats.pearsonr(df_final['compound scores'], df_final['Day Change Percentage'])
    print("Pearson Correlation coefficient:"+str(r[0]))
    return r[0]

# Define a function to get correlation coefficient between news' sentiment score and stock trading volume change of companies
def corr_coefficient(company, ticker):
    news_df=get_news(Company=company)
    sentiment_df=get_sentiment_score(news_df)
    start_d=sentiment_df['date'][0]
    end_d=sentiment_df['date'][len(sentiment_df)-1]
    volume_df=get_volume(stocks=ticker,start_date=start_d,stop_date=end_d)
    r=get_correlation_coefficient(sentiment_df,volume_df)
    return r

# Define a function called corr_df to automate the whole sentiment analysis process
def corr_df(companylist,tickers):
  corr_list=[]
  for i in range(len(companylist)):
      corr_list.append(corr_coefficient(companylist[i],[tickers[i]]))
  corr_df=pd.DataFrame(list(zip(companylist,corr_list)),columns=['company','corr_value'])
  return corr_df

# 11 companies sample 
df1=corr_df(['RBC','Microsoft','Amazon','CVR Energy','OXY','Coinbase','IBM','Alpha Metallurgical','Peabody Energy','AAPL','Consol Energy'],['RY','MSFT','AMZN','CVI','OXY','COIN','IBM','AMR','BTU','AAPL','CEIX'])

# Display correlation coefficient between trading volume change and Vader sentiment scores of selected companies
df1

# ETF sentiment analysis
# 5 interested ETFs
df2=corr_df(['iShares US Healthcare','iShares US Consumer Goods','iShares S&P 500 Growth','ARKK','ARKW'],['IYH','IYK','IVW','ARKK','ARKW'])

# Display correlation coefficient between trading volume change and Vader sentiment scores of selected ETFs
df2

"""# Volatility Analysis"""

# Google news api library install
!pip install gnews

# Import required library
from gnews import GNews
import pandas as pd
import datetime
import nltk
nltk.download('vader_lexicon')
from nltk.sentiment.vader import SentimentIntensityAnalyzer
sid = SentimentIntensityAnalyzer()
import re
import string
import unicodedata
!pip install unidecode
import unidecode
!pip install yfinance
import yfinance as yf
import scipy.stats as stats

# Define a function called get_news to pull news from Google News API
def get_news(Language='en',Country='US', Company='Microsoft'): 
# Google news setting
    google_news = GNews(language=Language, country=Country)
    news = google_news.get_news(Company)
    news_title=[]
    news_text=[]
    news_date=[]
    news_author=[]
    for i in range(len(news)):
        article = google_news.get_full_article(
        news[i]['url'])
        if article != None: 
            news_title.append(article.title)
            news_text.append(article.text)
            news_author.append(article.authors)
            news_date.append(news[i]['published date'])
    news_df=pd.DataFrame(list(zip(news_title,news_date,news_author,news_text)),columns=['title','publish date', 'author','text'])
    news_df['datetime']=news_df['publish date'].apply(lambda x: datetime.datetime.strptime(x, '%a, %d %b %Y %H:%M:%S %Z'))
    news_df=news_df.drop(columns=['publish date'])
    news_df['date']=news_df['datetime'].apply(lambda x: x.date())
    news_df=news_df.sort_values('date', ascending=False)
    return news_df

# Define a function called get_sentiment_score to score the polarity of news
def get_sentiment_score(news_df):
    def preprocess(text):
        text=text.lower()
        text=re.sub(r'[^\w\s]', '', text)
        text=unidecode.unidecode(text)
        text=re.sub(r'\d+', '', text)
        return text
    news_df['text']=news_df['text'].apply(lambda x: preprocess(x))
    news_df['polarity scores']=news_df['text'].apply(lambda x: sid.polarity_scores(x))
    news_df['compound scores']=news_df['polarity scores'].apply(lambda x: x['compound'])
    news_df_grouped=news_df.groupby('date')
    mean_df=news_df_grouped.mean()
    mean_df=mean_df.reset_index()
    return mean_df

# Define a function called get_volume to get stock price from yahoo finance
def get_volume(stocks=['MSFT'],start_date = '2022-05-01',stop_date = '2022-06-30'):
    stock_list = " ".join(stocks)
    table = yf.download(stock_list, start=start_date, end=stop_date)['Adj Close']
    df_stock=pd.DataFrame(table)
    df_stock=df_stock.reset_index().rename(columns={'Adj Close':'Close Price'})
    df_stock['date']=df_stock['Date'].apply(lambda x:x.date())
    df_stock.drop(columns=['Date'])
    return df_stock

# Define a function called get_correlation_coefficient to investigate the relationship between the sentiment scores of news and stock return
def get_volatility(mean_df,df_stock):
    df_final=pd.merge(mean_df,df_stock,how='left',on='date').dropna()
    df_final['Close Price Last Day']=df_final['Close Price'].shift(1)
    df_final['Return']=(df_final['Close Price']-df_final['Close Price Last Day'])/df_final['Close Price Last Day']
    df_final=df_final.dropna()
    volatility_stock=df_final['Return'].std()
    volatility_polarity=df_final['compound scores'].std()
    a=[volatility_stock,volatility_polarity]
    return a

# Define a function to get volatility of news' sentiment score and stock price of companies
def vola_value(company, ticker):
    news_df=get_news(Company=company)
    sentiment_df=get_sentiment_score(news_df)
    start_d=sentiment_df['date'][0]
    end_d=sentiment_df['date'][len(sentiment_df)-1]
    volume_df=get_volume(stocks=ticker,start_date=start_d,stop_date=end_d)
    r=get_volatility(sentiment_df,volume_df)
    return r

# Define a function called corr_df to automate the whole sentiment analysis process
def vola(companylist,tickers):
  stockv_list=[]
  polarityv_list=[]
  for i in range(len(companylist)):
      stockv_list.append(vola_value(companylist[i],[tickers[i]])[0])
      polarityv_list.append(vola_value(companylist[i],[tickers[i]])[1])
  vola_df=pd.DataFrame(list(zip(companylist,stockv_list,polarityv_list)),columns=['company','volatility of stocks','volatility of sentiment scores'])
  return vola_df

# 11 companies sample 
#df1=vola(['RBC','Microsoft','Amazon','CVR Energy','OXY','Coinbase','IBM','Alpha Metallurgical','Peabody Energy','AAPL','Consol Energy'],['RY','MSFT','AMZN','CVI','OXY','COIN','IBM','AMR','BTU','AAPL','CEIX'])

# Display 11 companies volatility
#df1

# ETF sentiment analysis
# 5 interested ETFs
df2=vola(['iShares US Healthcare','iShares US Consumer Goods','iShares S&P 500 Growth','ARKK','ARKW'],['IYH','IYK','IVW','ARKK','ARKW'])

# Display volatility dataframe of the ETFs
df2

# Estimate the correlation coefficient of volatility of ETF and sentiment scores
r = stats.pearsonr(df2['volatility of stocks'], df2['volatility of sentiment scores'])
print('Correlation Coefficient:'+str(r[0]))

"""# FinBERT Sentiment Analysis"""

# google news api library install
!pip install gnews
!pip install pygooglenews
!pip install torch
!pip install transformers

# import required library
from gnews import GNews
import pandas as pd
import datetime
import nltk
nltk.download('vader_lexicon')
from nltk.sentiment.vader import SentimentIntensityAnalyzer
sid = SentimentIntensityAnalyzer()
import re
import string
import unicodedata
!pip install unidecode
import unidecode
!pip install yfinance
import yfinance as yf
import scipy.stats as stats
from transformers import BertTokenizer, BertForSequenceClassification
from transformers import pipeline
import torch


### Setting up FinBert
from transformers import AutoTokenizer, AutoModelForSequenceClassification 
# Tokenizer
tokenizer = AutoTokenizer.from_pretrained("ProsusAI/finbert")
# Model
model = AutoModelForSequenceClassification.from_pretrained("ProsusAI/finbert")

# Define function to extract news from google news API
def get_news(Language='en',Country='US', Company='Microsoft'): 
# google news setting
    google_news = GNews(language=Language, country=Country)
    news = google_news.get_news(Company)
    news_title=[]
    news_text=[]
    news_date=[]
    news_author=[]
    for i in range(len(news)):
        article = google_news.get_full_article(
        news[i]['url'])
        if article != None: 
            news_title.append(article.title)
            news_text.append(article.text)
            news_author.append(article.authors)
            news_date.append(news[i]['published date'])
    news_df=pd.DataFrame(list(zip(news_title,news_date,news_author,news_text)),columns=['title','publish date', 'author','text'])
    news_df['datetime']=news_df['publish date'].apply(lambda x: datetime.datetime.strptime(x, '%a, %d %b %Y %H:%M:%S %Z'))
    news_df=news_df.drop(columns=['publish date'])
    news_df['date']=news_df['datetime'].apply(lambda x: x.date())
    news_df=news_df.sort_values('date', ascending=False)
    return news_df

# Load news of Amazon
amazon_news=get_news(Company='Amazon')

# Define FinBert Model 
finbert = BertForSequenceClassification.from_pretrained('yiyanghkust/finbert-tone',num_labels=3)
tokenizer = BertTokenizer.from_pretrained('yiyanghkust/finbert-tone')

nlp = pipeline("sentiment-analysis", model=finbert, tokenizer=tokenizer)

# Define FinBERT function to estimate the polarity of the text
def FinBERT(text):
  #text=text.lower()
  #text=re.sub(r'[^\w\s]', '', text)
  #text=unidecode.unidecode(text)
  #text=re.sub(r'\d+', '', text)
  text=text[:500]
  
  output=nlp(text)
  polarity= output[0].get('label')
  return polarity

def FinBERT_raw(text):
 # text=text.lower()
 # text=re.sub(r'[^\w\s]', '', text)
 # text=unidecode.unidecode(text)
 # text=re.sub(r'\d+', '', text)
  text=text[:500]
  
  output=nlp(text)
  raw= output[0].get('score')
  return raw

# Estimate the polarities of Amazon news articles
amazon_news['text']

amazon_news['Finbert Score']=amazon_news['title'].apply(lambda x: FinBERT(x))
amazon_news.drop(columns=['author','datetime','ESG_score'])

"""# FinBERT-ESG"""

# Define FinBERT-ESG model
from transformers import BertTokenizer, BertForSequenceClassification, pipeline

finbert = BertForSequenceClassification.from_pretrained('yiyanghkust/finbert-esg',num_labels=4)
tokenizer = BertTokenizer.from_pretrained('yiyanghkust/finbert-esg')

# Test the ESG result
esg_nlp = pipeline("text-classification", model=finbert, tokenizer=tokenizer)
results = esg_nlp('Rhonda has been volunteering for several years for a variety of charitable community programs.')
print(results) # [{'label': 'Social', 'score': 0.9906041026115417}]

# Classify the ESG category and estimate ESG score of amazon news articles
nlp(amazon_news['title'].values[0])

def ESG (text):
  text=text[:500]
  
  output=esg_nlp(text)
  polarity= output[0].get('label')
  score=output[0].get('score')
  return polarity, score

ESG(amazon_news['text'].values[0])

amazon_news['ESG_score']=amazon_news['text'].apply(lambda x: ESG(x))
amazon_news.head(5)

# Extract useful information for report use
amazon_news.head(5).drop(columns=['datetime', 'author','text'])